...
enemy[0] = Enemy(WIDTH/2-385, 105, "sm")
enemy[1] = Enemy(WIDTH/2-200, 105, "sm")
enemy[2] = Enemy(WIDTH/2, 105, "boss")
enemy[3] = Enemy(WIDTH/2+200, 105, "sm")
enemy[4] = Enemy(WIDTH/2+385, 105, "sm")
...
while running:
    if Story_Game:
        screen = pygame.display.set_mode((WIDTH,HEIGHT))
        fuc.draw_story("Level_5", Keyboard)
        Story_Game = False
        screen = pygame.display.set_mode((WIDTH,HEIGHT))
    if End_Game:
        End_Game = False
    ...
    # 敵人生成倒數
    Enemy_Rez_Time -= 1
    if Enemy_Rez_Time == 0 and score[3] != 0:
        for i in range(enemy_number):
            if score[i+1] < 50:  # 分數
                score[i+1] += 10
                score[3] -= 10
            if enemy_is_exist[i] == False:  # 召喚被打死的小飛機
                all_sprites.add(enemy[i])
                enemy_group.add(enemy)
                enemy_is_exist[i] = True
            Enemy_Rez_Time = 300
    ...
    if delay == -1 and result == 'Win':
        End_Game = True
        once_win = True
        fuc.draw_end(result, score[0], enemy_total_score, "Level_6.py", Keyboard, once_win)
        screen = pygame.display.set_mode((WIDTH,HEIGHT))
        delay = 0
        cap.release()
    ...
    if End_Game == False and Story_Game == False:
        ...
        # 0:player，1:enemy
        fuc.draw_health(screen, 150, score[0], WIDTH - 180, HEIGHT - 40)
        ...
        fuc.draw_health(screen, 200, score[3], 20, 100)
