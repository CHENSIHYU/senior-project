...
# 載入圖片
player_img = pygame.image.load(os.path.join("img",'plColor.png')).convert()
plane01R_img = pygame.image.load(os.path.join("img", "plColor_R.png")).convert()    #飛機右轉
plane01L_img = pygame.image.load(os.path.join("img", "plColor_L.png")).convert()    #飛機左轉
...
class Player(pygame.sprite.Sprite):
    ...
    def animate(self,newPo, oldPo):
        if oldPo>newPo: #向左
            self.image = pygame.transform.scale(plane01L_img, (120,80))
        elif oldPo<newPo:   #向右
            self.image = pygame.transform.scale(plane01R_img, (120,80))
        else:   #回到原圖片
            self.image = pygame.transform.scale(player_img, (120,80))
        self.image.set_colorkey(BLACK)
