...
def enemy_shoot():
    for i in range(enemy_number):
        if score[i+1] != 0:
            enemy_bullet1 = Enemy_Bullet(enemy[i].rect.centerx+15, enemy[i].rect.bottom)
            all_sprites.add(enemy_bullet1)
            enemy_bullets.add(enemy_bullet1)
            enemy_bullet2 = Enemy_Bullet(enemy[i].rect.centerx-15, enemy[i].rect.bottom)
...
while running:
    if Story_Game:
        screen = pygame.display.set_mode((WIDTH,HEIGHT))
        fuc.draw_story("Level_3", Keyboard)
        Story_Game = False
        screen = pygame.display.set_mode((WIDTH,HEIGHT))
    if End_Game:
        End_Game = False
    ...
    if enemy_left_time >= 0:
        for i in range(enemy_number):
            ...
            enemyX = enemy[i].rect.centerx  # 敵人目前位置
            enemy[i].animate(1)
        enemy_left_time -= 1
        if enemy_left_time == 0:
    ...
    if delay == -1 and result == 'Win':
        End_Game = True
        once_win = True
        fuc.draw_end(result, score[0], enemy_total_score, "Level_4.py", Keyboard, once_win)
        screen = pygame.display.set_mode((WIDTH,HEIGHT))
        delay = 0
        cap.release()
